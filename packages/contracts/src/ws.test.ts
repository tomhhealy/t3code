...
it.effect("accepts skills.listInstalled requests", () =>
  Effect.gen(function* () {
    const parsed = yield* decodeWebSocketRequest({
      id: "req-skills-1",
      body: {
        _tag: WS_METHODS.skillsListInstalled,
        codexHomePathOverride: "/tmp/codex-home",
      },
    });
    assert.strictEqual(parsed.body._tag, WS_METHODS.skillsListInstalled);
  }),
);
